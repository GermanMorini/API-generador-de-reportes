...
public class Controller {
    ...
    public ResponseEntity<byte[]> genRepo1(@RequestBody Reporte1 params) {
        ...
        JasperReport repo = JasperCompileManager
                .compileReport(getClass().getResourceAsStream("/templates/report1.jrxml"));
    ...
    public ResponseEntity<byte[]> genRepo2(@RequestBody Reporte2 params) {
        ...
        JasperReport repo = JasperCompileManager
                .compileReport(getClass().getResourceAsStream("/templates/report2.jrxml"));
        JasperPrint jp = JasperFillManager.fillReport(repo, p, new JREmptyDataSource());
        ...
        params.forEach((k, v) -> {
            if (v == null)
                throw new NullPointerException(
                        "No se pudo construir el PDF a partir de estos valores: (" + k + " - " + v + ")");
        });
    }
    ...
    static private class Reporte2 {
        private String dni, nombre_estudiante, clave, legajo, carrera, plan_estudios, fecha_examen, hora_apertura,
                validacion;
    }
}
